...
class Billing:
    base_app_template = 'Billing/base_billing.html'
    objects_per_page = WORDS_PER_PAGE
    raise_exception = True


class LegalForm(Billing):
    model = LegalForm
    form_model = LegalFormForm
    object_redirect_url = 'lforms_list_url'
    function_name = object_redirect_url


class LegalFormList(LegalForm, ObjectsListMixin, View):
    template_name = 'Billing/lforms.html'


class LegalFormCreate(LegalForm, LoginRequiredMixin, ObjectCreateMixin, View):
    object_create_url = 'lform_create_url'


class LegalFormDetails(LegalForm, ObjectDetailsMixin, View):
    template_name = 'Billing/lform_details.html'


class LegalFormUpdate(LegalForm, LoginRequiredMixin, ObjectUpdateMixin, View):
    pass


class LegalFormDelete(LegalForm, LoginRequiredMixin, ObjectDeleteMixin, View):
    pass
...
class Org(Billing):
    model = Org
    form_model = OrgForm
    object_redirect_url = 'orgs_list_url'
    function_name = object_redirect_url


class OrgList(Org, ObjectsListMixin, View):
    template_name = 'Billing/orgs.html'


class OrgCreate(Org, LoginRequiredMixin, ObjectCreateMixin, View):
    object_create_url = 'org_create_url'


class OrgDetails(Org, ObjectDetailsMixin, View):
    template_name = 'Billing/org_details.html'


class OrgUpdate(Org, LoginRequiredMixin, ObjectUpdateMixin, View):
    pass


class OrgDelete(Org, LoginRequiredMixin, ObjectDeleteMixin, View):
    pass
